...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " `args.validation_prompt` multiple times: `args.validation_repeats`."
        ),
    )
    parser.add_argument(
        "--rank",
...
def main(args):
    ...
    train_dataset = DreamBoothDatasetWithMask(
    ...
    validation_dataset = DreamBoothDatasetWithMask(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                prompts = batch["instance_prompts"]
                if args.with_prior_preservation:
                    prompts += batch["class_prompts"]
                ...
                image_tensors = batch["instance_image_tensors"].to(dtype=vae.dtype)
                if args.with_prior_preservation:
                    class_image_tensors = batch["class_image_tensors"].to(dtype=vae.dtype)
                    image_tensors = torch.cat([image_tensors, class_image_tensors], dim=0)
                image_latents = vae.encode(image_tensors).latent_dist.sample()
                image_latents = (image_latents - vae_config_shift_factor) * vae_config_scaling_factor
                image_latents = image_latents.to(dtype=weight_dtype)

                masked_image_tensors = batch["instance_masked_image_tensors"].to(dtype=vae.dtype)
                if args.with_prior_preservation:
                    class_masked_image_tensors = batch["class_masked_image_tensors"].to(dtype=vae.dtype)
                    masked_image_tensors = torch.cat([masked_image_tensors, class_masked_image_tensors], dim=0)
                masked_image_latents = vae.encode(masked_image_tensors).latent_dist.sample()
                masked_image_latents = (masked_image_latents - vae.config.shift_factor) * vae.config.scaling_factor

                mask_tensors = batch["instance_mask_tensors"]
                if args.with_prior_preservation:
                    class_mask_tensors = batch["class_mask_tensors"]
                    mask_tensors = torch.cat([mask_tensors, class_mask_tensors], dim=0)
                ...
                mask_latent = mask_tensors.view(
                    image_latents.shape[0], image_latents.shape[2], vae_scale_factor, image_latents.shape[3], vae_scale_factor
                )  # batch_size, height, 8, width, 8
                mask_latent = mask_latent.permute(0, 2, 4, 1, 3)  # batch_size, 8, 8, height, width
                mask_latent = mask_latent.reshape(
                    image_latents.shape[0], vae_scale_factor * vae_scale_factor, image_latents.shape[2], image_latents.shape[3]
                ...
                timesteps = noise_scheduler_copy.timesteps[indices].to(device=image_latents.device)

                # Add noise according to flow matching.
                # zt = (1 - texp) * x + texp * z1
                sigmas = get_sigmas(timesteps, n_dim=image_latents.ndim, dtype=image_latents.dtype)
                ...
                mask_latent = FluxFillPipeline._pack_latents(
                    mask_latent,
                    batch_size=image_latents.shape[0],
                ...
                masked_image_latents = torch.cat((masked_image_latents, mask_latent), dim=-1)
                ...
                model_pred = FluxFillPipeline._unpack_latents(
                    model_pred,
                    height=image_latents.shape[2] * vae_scale_factor,
                    width=image_latents.shape[3] * vae_scale_factor,
    ...
    if accelerator.is_main_process:
        ...
        # run inference
        images = []
        if args.validation_prompt and args.validation_repeats > 0:
            images = log_validation(
                validation_dataset=validation_dataset,
                pipeline=pipeline,
                args=args,
